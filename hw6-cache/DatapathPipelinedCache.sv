...
module DatapathPipelinedCache (
  ...
  // Store rs2 after load is handled by MX bypass of m_load_result in X (no WM).
  ...
  wire mem_stall = (m_mem_real && m_is_ld_mem && !dcache.RVALID)
                || (m_mem_real && m_is_st_mem && !dcache.BVALID);
  ...
  wire ar_hs = icache.ARVALID && icache.ARREADY;
  wire r_hs  = icache.RVALID && icache.RREADY;
  wire ififo_full = qv0 && qv1;
  logic icache_rready_w;

  always_comb begin
    icache_rready_w = False;
    if (icache.RVALID) begin
      if (!(g_valid && pipeline_stall)) icache_rready_w = True;
    end
  end
  assign icache.RREADY = icache_rready_w;
  ...
  always_comb begin
    icache.ARPROT = 3'b000;
    icache.ARADDR = seq_pc;
    if (rst || x_branch_taken) icache.ARVALID = False;
    else if (pipeline_stall) icache.ARVALID = False;
    else icache.ARVALID = !ififo_full;
  end

  cycle_status_e f_cycle_status;
  always_comb begin
    f_cycle_status = CYCLE_NO_STALL;
    if (icache.RVALID && !icache_rready_w) f_cycle_status = CYCLE_ICACHE_MISS;
  end
  ...
  always_comb begin
    m_load_result=0;
    if(m_lw) m_load_result=dcache.RDATA;
    else if(m_lh)  m_load_result=memory_state.alu_result[1]?{{16{dcache.RDATA[31]}},dcache.RDATA[31:16]}:{{16{dcache.RDATA[15]}},dcache.RDATA[15:0]};
    else if(m_lhu) m_load_result=memory_state.alu_result[1]?{16'b0,dcache.RDATA[31:16]}:{16'b0,dcache.RDATA[15:0]};
    else if(m_lb) case(memory_state.alu_result[1:0])
      2'b00:m_load_result={{24{dcache.RDATA[7]}},dcache.RDATA[7:0]};
      2'b01:m_load_result={{24{dcache.RDATA[15]}},dcache.RDATA[15:8]};
      2'b10:m_load_result={{24{dcache.RDATA[23]}},dcache.RDATA[23:16]};
      2'b11:m_load_result={{24{dcache.RDATA[31]}},dcache.RDATA[31:24]};
    endcase
    else if(m_lbu) case(memory_state.alu_result[1:0])
      2'b00:m_load_result={24'b0,dcache.RDATA[7:0]};
      2'b01:m_load_result={24'b0,dcache.RDATA[15:8]};
      2'b10:m_load_result={24'b0,dcache.RDATA[23:16]};
      2'b11:m_load_result={24'b0,dcache.RDATA[31:24]};
    endcase
  end
  ...
  always_comb begin
    dcache.ARPROT = 3'b000;
    dcache.AWPROT = 3'b000;
    dcache.ARADDR = x_dmem_addr;
    dcache.ARVALID = x_do_load && !mem_stall && !load_use_stall && !div_stall;
    dcache.AWADDR = x_dmem_addr;
    dcache.AWVALID = x_do_store && !mem_stall && !load_use_stall && !div_stall;
    dcache.WDATA  = x_st_wdata;
    dcache.WSTRB  = x_st_wstrb;
    dcache.WVALID = x_do_store && !mem_stall && !load_use_stall && !div_stall;
    dcache.RREADY = 1'b1;
    dcache.BREADY = 1'b1;
  end
  ...
  wire wb_dcache_miss = mem_stall && m_mem_real && m_is_ld_mem && !dcache.RVALID;
  always_ff @(posedge clk) begin
    if (rst)
      writeback_state <= '{pc:0,insn:0,cycle_status:CYCLE_RESET,rd_data:0,rd:0,we:0};
    else if (wb_dcache_miss)
  ...
  assign trace_writeback_pc           = w_valid ? writeback_state.pc   : 32'd0;
  assign trace_writeback_insn         = w_valid ? writeback_state.insn : 32'd0;
