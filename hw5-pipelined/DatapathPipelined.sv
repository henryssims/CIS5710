...
module DatapathPipelined (
  ...
  // Registered divider inputs: loaded once when a div enters Execute.
  // This breaks the combinational loop Verilator warns about.
  // Divider inputs driven combinationally from Execute — must be presented the
  // same cycle the div is in Execute so metadata and divider stay in sync.
  // The UNOPTFLAT warning is a false positive (no actual loop through alu_result
  // since div sets x_we=0 and never produces x_alu_result).
  ...
  always_comb begin
    div_pipeline_busy = 1'b0;
    div_dep_hazard = 1'b0;
    for (int s = 0; s < `DIVIDER_STAGES-1; s++) begin
  ...
  wire div_stall = d_is_div ? div_dep_hazard : div_pipeline_busy;

  // A ready divide result entering M only conflicts with a non-div instruction
  // currently in Execute, because a divide in Execute launches into the divider
  // and does not need the M slot this cycle.
  wire x_valid_nondiv = (execute_state.cycle_status == CYCLE_NO_STALL) && !x_is_div;
  wire div_insert_stall = div_result_valid && x_valid_nondiv;

  wire pipeline_stall = load_use_stall || div_stall || div_insert_stall;
  ...
  wire d_needs_load = ((d_uses_rs1 && d_rs1 == execute_state.insn[11:7])
                    || (d_uses_rs2 && d_rs2 == execute_state.insn[11:7]));
  ...
  always_ff @(posedge clk) begin
    if (rst)
    ...
    else if (div_stall || div_insert_stall)
      ...
      execute_state <= '{
        ...
        rs1_data:d_rs1_data_rf, rs2_data:d_rs2_data_rf, rs1:d_rs1, rs2:d_rs2, rd:d_rd
      };
  end
  ...
  // Divider-safe operand forwarding:
  // allow M->X only from non-div instructions; never feed an M-stage divide
  // result back into the divider combinationally.
  wire m_is_div = (memory_state.insn[6:0] == OpcodeRegReg)
               && (memory_state.insn[31:25] == 7'd1)
               && (memory_state.insn[14:12] >= 3'b100);
  wire mx_div_rs1 = m_we && !m_is_div && m_rd != 5'd0 && m_rd == execute_state.rs1;
  wire mx_div_rs2 = m_we && !m_is_div && m_rd != 5'd0 && m_rd == execute_state.rs2;
  ...
  always_comb begin
    ...
    case (x_opcode)
      ...
      OpcodeRegReg: begin
        ...
        else if(x_div)  begin x_we=0;div_dividend=x_div_rs1[31]?(~x_div_rs1+1):x_div_rs1;div_divisor=x_div_rs2[31]?(~x_div_rs2+1):x_div_rs2;end
        else if(x_divu) begin x_we=0;div_dividend=x_div_rs1;div_divisor=x_div_rs2;end
        else if(x_rem)  begin x_we=0;div_dividend=x_div_rs1[31]?(~x_div_rs1+1):x_div_rs1;div_divisor=x_div_rs2[31]?(~x_div_rs2+1):x_div_rs2;end
        else if(x_remu) begin x_we=0;div_dividend=x_div_rs1;div_divisor=x_div_rs2;end
      end
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      if (x_is_div && !pipeline_stall && !x_branch_taken) begin
        div_meta[0] <= '{
          valid:        1'b1,
          rd:           x_rd,
          pc:           execute_state.pc,
